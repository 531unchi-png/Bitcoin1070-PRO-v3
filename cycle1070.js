// Bitcoin1070 PRO v11.5 - BTC 実チャート延長 × 半減期 × 1070日理論
(() => {
  'use strict';
  ...
  function renderCompact() {
    const s = snapshot();
    const maps = buildRoadmaps(Math.max(1, currentMarket.price || 1));
    const summary = buildScenarioSummary(s, currentMarket, maps);
    ...
    if ($('homeScenarioBadge')) {
      $('homeScenarioBadge').textContent = summary.current.label;
      $('homeScenarioBadge').className = `cycle-badge ${summary.current.key}`;
    }
    if ($('homeScenarioReason')) $('homeScenarioReason').textContent = summary.current.reason[0] || '半減期と1070日理論から現在シナリオを判定します。';
  ...
  function renderPage(market) {
    ...
    const s = snapshot();
    const maps = buildRoadmaps(Math.max(1, market.price || 1));
    const state = buildScenarioSummary(s, market, maps);
    ...
    if ($('currentScenarioBadge')) {
      $('currentScenarioBadge').textContent = state.current.label;
      $('currentScenarioBadge').className = `cycle-badge ${state.current.key}`;
    }
    set('currentScenarioLabel', `現状判定：${state.current.label}シナリオ優勢`);
    set('currentScenarioReason', state.current.reason.slice(0, 2).join(' ／ ') || '市場状況とサイクル進捗から判定しています。');
  ...
  async function fetchBitcoinContext() {
    const cacheKey = 'bitcoin1070_cycle_context_v11_5';
    let cached = null;
    try { cached = JSON.parse(localStorage.getItem(cacheKey) || 'null'); } catch (_) {}
    const result = { price: Number(cached?.price) || 0, change: Number(cached?.change) || 0, fearValue: cached?.fearValue == null ? null : Number(cached.fearValue), fearLabel: cached?.fearLabel || '', cached: Boolean(cached) };
    ...
    try {
      ...
      if (price > 0) { result.price = price; result.change = change; result.cached = false; }
    } catch (error) { console.warn('BTC価格取得失敗、キャッシュ利用', error); }
    ...
    try {
      ...
      if (Number.isFinite(fearValue)) { result.fearValue = fearValue; result.fearLabel = fearLabel; result.cached = false; }
    } catch (error) { console.warn('Fear & Greed取得失敗', error); }

    try { localStorage.setItem(cacheKey, JSON.stringify({ ...result, fetchedAt: new Date().toISOString() })); } catch (_) {}
    return result;
  }

  async function fetchHistoricalBTC() {
    const cacheKey = 'bitcoin1070_btc_history_v11_5';
    let cached = null;
    try { cached = JSON.parse(localStorage.getItem(cacheKey) || 'null'); } catch (_) {}

    try {
      const response = await fetch('https://api.coingecko.com/api/v3/coins/bitcoin/market_chart?vs_currency=jpy&days=max&interval=daily', { cache: 'no-store' });
      if (!response.ok) throw new Error(`history error ${response.status}`);
      const data = await response.json();
      const prices = Array.isArray(data?.prices) ? data.prices : [];
      const filtered = prices.filter((_, index) => index % 7 === 0 || index === prices.length - 1)
        .map(([x, y]) => ({ x, y: Number(y) }))
        .filter(item => item.y > 0 && new Date(item.x).getFullYear() >= 2016);
      if (filtered.length) {
        try { localStorage.setItem(cacheKey, JSON.stringify({ points: filtered, fetchedAt: new Date().toISOString() })); } catch (_) {}
        return { points: filtered, cached: false };
      }
      throw new Error('empty history');
    } catch (error) {
      console.warn('BTC履歴取得失敗、キャッシュ利用', error);
      return { points: Array.isArray(cached?.points) ? cached.points : [], cached: true };
    }
  }
  ...
  async function initPage() {
    ...
    renderRealChart(history, state.maps);
  }
