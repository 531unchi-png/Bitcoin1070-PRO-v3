...
function sanitizeCryptoTaxRecords(value){
  ...
  const annual=Array.isArray(value.annual)?value.annual:[],trades=Array.isArray(value.trades)?value.trades:[],withdrawals=Array.isArray(value.withdrawals)?value.withdrawals:[],openingBasis=value.openingBasis||{},baseIncome=value.baseIncome||{},coverage=value.coverage||{};
  if(annual.length>30||trades.length>25000||withdrawals.length>1000||!openingBasis||typeof openingBasis!=="object"||Array.isArray(openingBasis)||!baseIncome||typeof baseIncome!=="object"||Array.isArray(baseIncome)||!coverage||typeof coverage!=="object"||Array.isArray(coverage))throw new Error("暗号資産税データが多すぎるか不正です");
  ...
  const cleanBasis={},cleanIncome={},cleanCoverage={};for(const [key,val]of Object.entries(openingBasis)){if(!/^20\d\d:[A-Z0-9]{2,20}$/.test(key))throw new Error("年始原価のキーが不正です");cleanBasis[key]=safeNumber(val,"年始取得原価");}for(const [key,val]of Object.entries(baseIncome)){if(!/^20\d\d$/.test(key))throw new Error("課税所得の年が不正です");cleanIncome[key]=safeNumber(val,"課税所得");}for(const [key,val]of Object.entries(coverage)){if(!/^20\d\d$/.test(key)||typeof val!=="boolean")throw new Error("取引網羅性が不正です");cleanCoverage[key]=val;}return{annual:cleanAnnual,trades:cleanTrades,withdrawals:cleanWithdrawals,openingBasis:cleanBasis,baseIncome:cleanIncome,coverage:cleanCoverage};
}
function loadCryptoTaxRecords(){try{const raw=localStorage.getItem(activeStorageKey(CRYPTO_TAX_KEY));return raw?sanitizeCryptoTaxRecords(JSON.parse(raw)):{annual:[],trades:[],withdrawals:[],openingBasis:{},baseIncome:{},coverage:{}};}catch(error){console.error("暗号資産税データ読込エラー",error);throw error;}}
