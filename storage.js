// Bitcoin1070 PRO v12.3 - Storage Manager (backward compatible / validated restore)
...
function loadTransactionsFromStorage(){try{const raw=localStorage.getItem(STORAGE_KEYS.TRANSACTIONS);if(!raw)return[];const data=JSON.parse(raw);return Array.isArray(data)?data.map(sanitizeTransaction):[];}catch(e){console.error("取引台帳読込エラー:",e);return[];}}
function saveTransactionsToStorage(items){const clean=items.slice(0,5000).map(sanitizeTransaction);localStorage.setItem(STORAGE_KEYS.TRANSACTIONS,JSON.stringify(clean));return clean;}
