...
function sanitizeCryptoTaxRecords(value){
  ...
  const annual=Array.isArray(value.annual)?value.annual:[],trades=Array.isArray(value.trades)?value.trades:[],withdrawals=Array.isArray(value.withdrawals)?value.withdrawals:[],rakuten=Array.isArray(value.rakuten)?value.rakuten:[],openingBasis=value.openingBasis||{},baseIncome=value.baseIncome||{},coverage=value.coverage||{};
  if(annual.length>30||trades.length>25000||withdrawals.length>1000||rakuten.length>30||!openingBasis||typeof openingBasis!=="object"||Array.isArray(openingBasis)||!baseIncome||typeof baseIncome!=="object"||Array.isArray(baseIncome)||!coverage||typeof coverage!=="object"||Array.isArray(coverage))throw new Error("暗号資産税データが多すぎるか不正です");
