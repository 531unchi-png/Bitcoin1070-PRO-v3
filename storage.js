...
function sanitizeAsset(asset){if(!asset||typeof asset!=="object")throw new Error("資産データが不正です");const type=safeString(asset.type,"type",20);if(!["crypto","jp","us","fund"].includes(type))throw new Error("資産typeが不正です");const out={type,name:safeString(asset.name,"name",120),symbol:safeString(asset.symbol,"symbol",40),amount:safeNumber(asset.amount,"amount"),cost:safeNumber(asset.cost,"cost",{nullable:true})};if(asset.coinGeckoId!=null&&asset.coinGeckoId!=="")out.coinGeckoId=safeString(asset.coinGeckoId,"coinGeckoId",120);if(asset.yahooSymbol!=null&&asset.yahooSymbol!=="")out.yahooSymbol=safeString(asset.yahooSymbol,"yahooSymbol",80);if(asset.costJpy!=null&&asset.costJpy!=="")out.costJpy=safeNumber(asset.costJpy,"costJpy");if(asset.acquisitionUsdJpy!=null&&asset.acquisitionUsdJpy!=="")out.acquisitionUsdJpy=safeNumber(asset.acquisitionUsdJpy,"acquisitionUsdJpy");if(type==="fund"){out.navJpy=safeNumber(asset.navJpy,"基準価額",{positive:true});if(!/^\d{4}-\d{2}-\d{2}$/.test(String(asset.navDate||""))||!Number.isFinite(Date.parse(`${asset.navDate}T00:00:00Z`))||asset.navDate>new Date().toISOString().slice(0,10))throw new Error("基準価額の日付が不正です");out.navDate=asset.navDate;out.accountType=["nisa","taxable","unknown"].includes(asset.accountType)?asset.accountType:"unknown";}return out;}
...
function applyLedgerTransaction(currentAssets,currentCash,input){
  ...
  if(!asset){if(entry.type==="fund")throw new Error("投資信託は資産編集画面で基準価額を登録してください");asset=sanitizeAsset({type:entry.type,symbol:entry.symbol,name:entry.name,amount:0,cost:null,...(input.coinGeckoId?{coinGeckoId:input.coinGeckoId}:{}),...(input.yahooSymbol?{yahooSymbol:input.yahooSymbol}:{})});assets.push(asset);}
...
function importAppData(file){return new Promise((resolve,reject)=>{const reader=new FileReader();reader.onload=()=>{try{const data=JSON.parse(reader.result),schema=data.schemaVersion;if(schema!==undefined&&![4,5,6].includes(schema))throw new Error(`未対応のバックアップ形式です (schemaVersion: ${schema})`);if(!Array.isArray(data.assets))throw new Error("資産データの形式が不正です");const assets=data.assets.map(sanitizeAsset),history=sanitizeHistory(data.history),transactions=Array.isArray(data.transactions)?data.transactions.map(sanitizeTransaction):[];let cashBalance=null;if(data.cashBalance!=null)cashBalance=safeNumber(data.cashBalance,"cashBalance");const assetSnapshots=schema===6?sanitizeAssetSnapshots(data.assetSnapshots):[],strategyLimits=schema===6&&data.strategyLimits!=null?sanitizeStrategyLimits(data.strategyLimits):null;resolve({assets,history,cashBalance,transactions,assetSnapshots,strategyLimits,metadata:backupSummary(data)});}catch(e){reject(e);}};reader.onerror=()=>reject(new Error("ファイルの読み込みに失敗しました"));reader.readAsText(file);});}
