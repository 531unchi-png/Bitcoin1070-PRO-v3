...
function sanitizeCryptoTaxRecords(value){
  ...
  const annual=Array.isArray(value.annual)?value.annual:[],trades=Array.isArray(value.trades)?value.trades:[],withdrawals=Array.isArray(value.withdrawals)?value.withdrawals:[],rakuten=Array.isArray(value.rakuten)?value.rakuten:[],rakutenPoints=Array.isArray(value.rakutenPoints)?value.rakutenPoints:[],openingBasis=value.openingBasis||{},baseIncome=value.baseIncome||{},coverage=value.coverage||{};
  if(annual.length>30||trades.length>25000||withdrawals.length>1000||rakuten.length>30||rakutenPoints.length>30||!openingBasis||typeof openingBasis!=="object"||Array.isArray(openingBasis)||!baseIncome||typeof baseIncome!=="object"||Array.isArray(baseIncome)||!coverage||typeof coverage!=="object"||Array.isArray(coverage))throw new Error("暗号資産税データが多すぎるか不正です");
  ...
  const cleanAnnual=annual.map(item=>{const year=safeNumber(item.year,"報告年");if(!Number.isInteger(year)||year<2018||year>2100||!Array.isArray(item.rows)||item.rows.length>200)throw new Error("年間報告書が不正です");return {year,jpyFee:item.jpyFee==null?null:safeNumber(item.jpyFee,"JPY支払手数料"),rows:item.rows.map(row=>{const out={symbol:safeString(row.symbol,"通貨",20)};if(!/^[A-Z0-9]{2,20}$/.test(out.symbol)||out.symbol==="JPY")throw new Error("通貨が不正です");for(const key of fields)out[key]=safeNumber(row[key],key);return out;})};});
  if(new Set(cleanAnnual.map(x=>x.year)).size!==cleanAnnual.length)throw new Error("報告年が重複しています");
  const seen=new Set(),cleanTrades=trades.map(item=>{const id=safeString(item.id,"取引ID",120),pair=safeString(item.pair,"通貨ペア",40),symbol=safeString(item.symbol,"通貨",20),quote=safeString(item.quote,"建値通貨",20),date=safeString(item.date,"取引日時",40),kind=safeString(item.kind,"売買",4),venue=item.venue==null?'exchange':safeString(item.venue,"取引場所",10);if(!Number.isFinite(Date.parse(date))||!["BUY","SELL"].includes(kind)||!["exchange","dealer"].includes(venue)||seen.has(id))throw new Error("約定履歴が不正です");seen.add(id);const fee=Number(item.fee);if(!Number.isFinite(fee))throw new Error("手数料が不正です");return{id,pair,symbol,quote,kind,date,quantity:safeNumber(item.quantity,"数量",{positive:true}),price:safeNumber(item.price,"価格",{positive:true}),fee,venue};});
  const cleanWithdrawals=withdrawals.map(item=>{const date=safeString(item.date,"出金日時",40),fee=safeNumber(item.fee,"出金手数料");if(!Number.isFinite(Date.parse(date)))throw new Error("出金日時が不正です");return{date,fee};});
  const cleanRakuten=rakuten.map(item=>{const year=safeNumber(item.year,"楽天ウォレットの年"),pointCount=safeNumber(item.pointCount,"ポイント交換件数"),pointQty=safeNumber(item.pointQty,"ポイント交換BTC数量"),buyCount=safeNumber(item.buyCount,"購入件数"),buyQty=safeNumber(item.buyQty,"購入BTC数量"),buyJpy=safeNumber(item.buyJpy,"購入円額");if(!Number.isInteger(year)||year<2018||year>2100||!Number.isInteger(pointCount)||!Number.isInteger(buyCount)||pointCount+buyCount<1||pointCount>25000||buyCount>25000||(!pointCount&&pointQty)||(!buyCount&&(buyQty||buyJpy))||(pointCount&&pointQty<=0)||(buyCount&&(buyQty<=0||buyJpy<=0)))throw new Error("楽天ウォレットの履歴が不正です");const pointEntries=Array.isArray(item.pointEntries)?item.pointEntries.map(entry=>{const date=safeString(entry.date,"交換日",8),qty=safeNumber(entry.qty,"交換数量",{positive:true});if(!/^\d\d\/\d\d\/\d\d$/.test(date)||Number(date.slice(0,2))+2000!==year)throw new Error("楽天ウォレットの交換日が不正です");return{date,qty};}):[];if(pointEntries.length&&pointEntries.length!==pointCount)throw new Error("ポイント交換明細が不足しています");return{year,pointCount,pointQty,buyCount,buyQty,buyJpy,pointEntries};});
  if(new Set(cleanRakuten.map(x=>x.year)).size!==cleanRakuten.length)throw new Error("楽天ウォレットの年が重複しています");
  const cleanRakutenPoints=rakutenPoints.map(item=>{const year=safeNumber(item.year,"交換年"),count=safeNumber(item.count,"交換件数"),qty=safeNumber(item.qty,"交換数量"),points=safeNumber(item.points,"使用ポイント数");if(!Number.isInteger(year)||year<2018||year>2100||!Number.isInteger(count)||count<=0||!Number.isInteger(points)||points<=0||qty<=0||!cleanRakuten.some(x=>x.year===year&&x.pointCount===count&&Math.abs(x.pointQty-qty)<1e-7))throw new Error("楽天ウォレットの照合結果が不正です");return{year,count,qty,points};});if(new Set(cleanRakutenPoints.map(x=>x.year)).size!==cleanRakutenPoints.length)throw new Error("交換年が重複しています");
  const cleanBasis={},cleanIncome={},cleanCoverage={};for(const [key,val]of Object.entries(openingBasis)){if(!/^20\d\d:[A-Z0-9]{2,20}$/.test(key))throw new Error("年始原価のキーが不正です");cleanBasis[key]=safeNumber(val,"年始取得原価");}for(const [key,val]of Object.entries(baseIncome)){if(!/^20\d\d$/.test(key))throw new Error("課税所得の年が不正です");cleanIncome[key]=safeNumber(val,"課税所得");}for(const [key,val]of Object.entries(coverage)){if(!/^20\d\d$/.test(key)||typeof val!=="boolean")throw new Error("取引網羅性が不正です");cleanCoverage[key]=val;}return{annual:cleanAnnual,trades:cleanTrades,withdrawals:cleanWithdrawals,rakuten:cleanRakuten,rakutenPoints:cleanRakutenPoints,openingBasis:cleanBasis,baseIncome:cleanIncome,coverage:cleanCoverage};
}
function loadCryptoTaxRecords(){try{const raw=localStorage.getItem(activeStorageKey(CRYPTO_TAX_KEY));return raw?sanitizeCryptoTaxRecords(JSON.parse(raw)):{annual:[],trades:[],withdrawals:[],rakuten:[],openingBasis:{},baseIncome:{},coverage:{}};}catch(error){console.error("暗号資産税データ読込エラー",error);throw error;}}
