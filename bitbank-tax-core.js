...
((root,factory)=>{const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;if(root)root.B1070BitbankTax=api;})(typeof window!=='undefined'?window:null,()=>{
  ...
  function parseAnnual(text,year){if(!Number.isInteger(Number(year))||year<2018||year>2100)throw Error('対象年を確認してください');const rows=csv(text),h=head(rows,['通貨名','年始数量','年末数量']);for(const col of ['buyQty','buyJpy','sellQty','sellJpy','fee'])if(!h.has(...reportCols[col]))throw Error(`年間報告書の${reportCols[col][0]}列がありません`);
    const parsed=[];for(let i=h.index+1;i<rows.length;i++){const r=rows[i],name=String(h.get(r,'通貨名')||'').trim();if(!name)continue;if(/^(合計|total)$/i.test(name))continue;const sym=symbol(name);if(!/^[A-Z0-9]{2,20}$/.test(sym))throw Error(`${i+1}行目の通貨名を認識できません: ${name}`);const data={symbol:sym};for(const [key,labels]of Object.entries(reportCols)){if(!h.has(...labels)&&!['btcBuyQty','btcBuyJpy','btcSellQty','btcSellJpy','inQty','outQty','lentQty','returnedQty','lending'].includes(key))throw Error(`${labels[0]}列がありません`);data[key]=decimal(h.get(r,...labels),`${i+1}行目の${labels[0]}`,{blankZero:true});}if(parsed.some(x=>x.symbol===sym))throw Error(`${sym}の行が重複しています`);parsed.push(data);}
    if(!parsed.length)throw Error('年間報告書に通貨別の行がありません');return {year:Number(year),rows:parsed};
  }
  ...
  function parseTrades(text){const rows=csv(text),h=head(rows,['取引ID','通貨ペア','売/買','数量','価格','取引日時']),parsed=[];
    for(let i=h.index+1;i<rows.length;i++){const r=rows[i],pair=String(h.get(r,'通貨ペア')||'').trim().toUpperCase();if(!pair)continue;const type=norm(h.get(r,'タイプ'));if(h.has('タイプ')&&!['現物','spot','指値','成行','limit','market',''].includes(type))throw Error(`${i+1}行目は現物以外の取引です`);const id=String(h.get(r,'取引ID')||'').trim();if(!id)throw Error(`${i+1}行目の取引IDが空です`);const match=pair.match(/^([A-Z0-9]+)[_/-]([A-Z0-9]+)$/);if(!match)throw Error(`${i+1}行目の通貨ペアが不正です: ${pair}`);const side=norm(h.get(r,'売/買')),kind=['買','買い','buy'].includes(side)?'BUY':['売','売り','sell'].includes(side)?'SELL':null;if(!kind)throw Error(`${i+1}行目の売買区分が不正です`);const quantity=decimal(h.get(r,'数量'),`${i+1}行目の数量`),price=decimal(h.get(r,'価格'),`${i+1}行目の価格`);if(quantity<=0||price<=0)throw Error(`${i+1}行目の数量・価格が不正です`);const fee=h.has('手数料')?decimal(h.get(r,'手数料'),`${i+1}行目の手数料`,{blankZero:true,signed:true}):0;parsed.push({id,pair,symbol:match[1],quote:match[2],kind,quantity,price,fee,date:japanDate(h.get(r,'取引日時'))});}if(!parsed.length)throw Error('約定行がありません（注文履歴ではなく約定履歴を選んでください）');return parsed;
  }
  ...
  function analyze(records,year,openingBasis={}){
    ...
    for(const reportRow of report.rows){const r=reportRow,c=summaries.get(r.symbol)||{buyQty:0,buyJpy:0,sellQty:0,sellJpy:0,buyFee:0,sellFee:0};for(const key of ['buyQty','buyJpy','sellQty','sellJpy'])if(!close(r[key],c[key]))issues.push(`${r.symbol}の${key}が年間報告書と約定CSVで一致しません`);if(!close(r.fee,c.buyFee+c.sellFee))issues.push(`${r.symbol}の支払手数料が約定CSVと一致しません（円建て手数料のみ対応）`);summaries.delete(r.symbol);
