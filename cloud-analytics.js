// Bitcoin1070 PRO v16.5 - cloud analytics adapter
(()=>{'use strict';
if(window.Bitcoin1070CloudAnalytics?.version==='16.5')return;
const VERSION='16.5';
const CONFIG=Object.freeze({provider:'cloudflare-web-analytics',token:'',enabled:true});
...
function load(){if(!CONFIG.enabled)return status();const t=token();state.configured=Boolean(t);if(!t){state.enabled=false;return status()}if(document.querySelector('script[data-cf-beacon]')){state.enabled=true;state.loaded=true;return status()}try{const s=document.createElement('script');s.defer=true;s.src='https://static.cloudflareinsights.com/beacon.min.js';s.dataset.cfBeacon=JSON.stringify({token:t});s.dataset.b1070CloudAnalytics='1';s.referrerPolicy='strict-origin-when-cross-origin';s.addEventListener('load',()=>{state.enabled=true;state.loaded=true;window.dispatchEvent(new CustomEvent('bitcoin1070:cloud-analytics-ready',{detail:status()}))},{once:true});s.addEventListener('error',()=>{state.enabled=false;state.error='load_failed'},{once:true});document.head.appendChild(s);state.enabled=true}catch(_){state.enabled=false;state.error='load_failed'}return status()}
