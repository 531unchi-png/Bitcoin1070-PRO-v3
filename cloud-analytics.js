...
const VERSION='16.5';
// Public Cloudflare Web Analytics beacon token for 531unchi-png.github.io.
// This is not a Cloudflare account/API credential.
...
function load(){if(!CONFIG.enabled)return status();const t=token();state.configured=Boolean(t);if(!t){state.enabled=false;return status()}if(document.querySelector('script[data-cf-beacon]')){state.enabled=true;state.loaded=true;return status()}try{const s=document.createElement('script');s.defer=true;s.type='module';s.src='https://static.cloudflareinsights.com/beacon.min.js';s.dataset.cfBeacon=JSON.stringify({token:t});s.dataset.b1070CloudAnalytics='1';s.referrerPolicy='strict-origin-when-cross-origin';s.addEventListener('load',()=>{state.enabled=true;state.loaded=true;state.error=null;window.dispatchEvent(new CustomEvent('bitcoin1070:cloud-analytics-ready',{detail:status()}))},{once:true});s.addEventListener('error',()=>{state.enabled=false;state.loaded=false;state.error='load_failed';window.dispatchEvent(new CustomEvent('bitcoin1070:cloud-analytics-error',{detail:status()}))},{once:true});document.head.appendChild(s);state.enabled=true}catch(_){state.enabled=false;state.loaded=false;state.error='load_failed'}return status()}
