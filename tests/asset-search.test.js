...
const context={console,globalThis:null};context.window=context;context.globalThis=context;vm.createContext(context);
...
(async()=>{let result=await api.search("7203",{holdings,sellOnly:true});assert.deepEqual(result.items.map(x=>x.symbol),["7203"]);result=await api.search("apple",{holdings,sellOnly:true});assert.equal(result.items.length,0);console.log("asset search tests passed");})().catch(error=>{console.error(error);process.exitCode=1;});
