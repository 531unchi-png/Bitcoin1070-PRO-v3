...
vm.createContext(context);vm.runInContext(`${fs.readFileSync('storage.js','utf8')}\nthis.api={loadAssetsFromStorage,loadCashBalance,saveCashBalance,importAppData};`,context);
assert.equal(context.api.loadAssetsFromStorage([])[0].symbol,'5803');assert.equal(context.api.loadCashBalance(),123456);
session.set('bitcoin1070_v13_demo','1');assert.equal(context.api.loadAssetsFromStorage([])[0].symbol,'BTC');context.api.saveCashBalance(700000);assert.equal(values.get('bitcoin1070_v12_1_cash_jpy'),'123456');
session.delete('bitcoin1070_v13_demo');assert.equal(context.api.loadCashBalance(),123456);
