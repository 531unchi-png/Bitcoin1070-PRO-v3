...
function collectChanges(){ document.querySelectorAll('#editor [data-index][data-field]').forEach(input=>{ const asset=assets[Number(input.dataset.index)]; if(!asset)return; const field=input.dataset.field; if(field==='acquisitionUsdJpy'){setUsAssetAcquisitionFx(asset,input.value.trim());return;} asset[field]=(field==='amount'||field==='cost')?Math.max(0,Number(input.value)||0):input.value.trim(); }); }
